...
def test_review_effective_pool_matches_carry_over_cases():
    ...
    # период «из будущего» (гонка на границе суток UTC, скачок часов) — no-op, как в
    # канонической нормализации: иначе джоба блокировок срежет потолок и заблокирует
    # оплатившего, пока сводка API и бар подписки показывают ему остаток
    assert _effective_pool(10 * gb, "2999-12", now, [("2999-12", 100 * gb)], 3 * gb) == 10 * gb
